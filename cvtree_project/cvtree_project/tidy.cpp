...
private:
	...
	void InitVectors()
	{
		...
		memset(one_l, 0, AA_NUMBER * sizeof(long)); // individual characters (counts how many times we see A, B, ... amino acids
		total = 0;
		total_l = 0;
		complement = 0;
	}

    // creates a buffer of length 5 that we can later use
...
public:
	...
	Bacteria(char* filename)
	{
		...
		{
			if (ch == '>') // if it's >, we can skip the line since this is meta data
			{
				while (fgetc(bacteria_file) != '\n'); // skip rest of line

				char buffer[LEN-1]; // LEN is defined at the top of the file (default is 6)
                // Instead of reading separately, create new word by appending to current one
